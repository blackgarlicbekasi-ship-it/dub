...
export const generatePaypalOAuthUrl = authPartnerActionClient.action(
  async ({ ctx }) => {
    ...
    if (!availablePayoutMethods.includes(PartnerPayoutMethod.paypal)) {
      throw new Error(
        `Your current country (${COUNTRIES[partner.country]}) is not supported for PayPal payouts. Please go to partners.ingat.cc/settings to update your country, or contact support.`,
      );
    }
